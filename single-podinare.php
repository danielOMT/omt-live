...
$podinar_agenda = get_field('podinar_agenda');
$podinar_zielgruppe = get_field('podinar_zielgruppe');
$podinar_zusammenfassung = get_field('podinar_zusammenfassung');
    ...
    <div id="inner-content" class="wrap clearfix no-hero">
        <div id="main" class="omt-row blog-single  clearfix" role="main">
            ...
            <?php  if (strlen($soundcloud)>0) { //SOUNDCLOUD ?>
                ...
                <?php } ?>
            <?php }
            if (strlen($podinar_beschreibung)>0) { print $podinar_beschreibung; }
            if (strlen($podinar_agenda)>0) { print $podinar_agenda; }
            if (strlen($podinar_zielgruppe)>0) { print $podinar_zielgruppe; } ?>

            <div class="podinar-abonnieren teaser-modul">
                <?php foreach ($podinar_abonnieren_optionen as $option) {?>
                    <a class="teaser-small abonnieren" target="_blank" href="<?php print $option['link'];?>"><img src="<?php print $option['bild']['url'];?>" alt="<?php print $option['bild']['alt'];?>" title="<?php print $option['bild']['alt'];?>"</a>
                <?php } ?>
            </div>

            <?php foreach($podinar_speakers as $podinar_speaker) {
