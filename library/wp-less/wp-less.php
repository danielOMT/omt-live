...
class wp_less {
	...
	public function get_cache_dir( $path = true ) {
		...
		if ( $path ) {
			$dir = apply_filters( 'wp_less_cache_path', path_join( get_template_directory(), 'wp-less-cache' ) );
			// create folder if it doesn't exist yet
			wp_mkdir_p( $dir );
		} else {
			$dir = apply_filters( 'wp_less_cache_url', path_join( get_template_directory_uri(), 'wp-less-cache' ) );
		}
