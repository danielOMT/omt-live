if ($('.webinare-filter-dropdown').length > 0) {
    $(".webinare-filter-dropdown").on('change', function () {
        event.preventDefault();

        $content = $('.content-flat');
        $selected = $('.webinare-filter-dropdown option:selected').val();
        ...
        $.ajax({
            url: wbfiljamz.ajax_url,
            data: {
                action: 'do_filter_webinare',
                category: $selected,
                nonce: wbfiljamz.nonce
            },
            type: 'post',
            dataType: 'json',
